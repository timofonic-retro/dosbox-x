...
public:
    ...
    std::string str_glide = section->Get_string("glide");
    lowcase(str_glide);
    if(str_glide == "true") {
    } else if(str_glide == "emu") {
        void VOODOO_Init(Section* sec); void VOODOO_Destroy(Section* sec);
        Section_prop * sec=static_cast<Section_prop *>(control->GetSection("pci"));
        if(sec && strcmp(sec->Get_string("voodoo"),"false")) { VOODOO_Destroy(sec); VOODOO_Init(sec); }
        else LOG_MSG("Glide:Set voodoo=auto, software or opengl.");
    } else if (str_glide == "false") {
        return;
    } else return;
    ...
    } else if(str_glide == "emu") LOG_MSG("Glide:Internal emulation");
	...
	FILE * ovl = NULL;
	char path[MAX_PATH];
	char ovl_path[MAX_PATH];

	if(str_glide == "emu")
		ovl=fopen("glide2x_emu.ovl", "rb");
	else
		ovl=fopen("glide2x.ovl", "rb");
	...
	if(!ovl)
	{
		...
		if(str_glide == "emu")
			sprintf(ovl_path,"%s\\%s",path,"glide2x_emu.ovl");
		else
			sprintf(ovl_path,"%s\\%s",path,"glide2x.ovl");

		ovl=fopen(ovl_path, "rb");
	}
...
*/
	...
	GrOriginLocation = param[5];
 	...
 	if(!glide.fullscreen[0]) UI_Shortcut(0);
	else {
		HWND GetHWND(void);
